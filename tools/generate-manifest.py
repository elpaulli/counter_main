...
# id -> (label, [source folders])
FILTERS = [
    ("experience", "the Experience", ["experience"]),
    ("design",     "the Design",     ["design"]),
    ("details",    "the Details",    ["details"]),
    ("menus",      "the Menus",      ["experience", "details"]),
    ("beverages",  "the Beverages",  ["beverages"]),
    ("team",       "the Team",       ["team"]),
]
...
def main():
    ...
    for filter_id, _label, folders in FILTERS:
    ...
    manifest = {
        "filters": [
            {"id": fid, "label": label, "folders": folders}
            for fid, label, folders in FILTERS
        ],
        "images": images,
    }
    ...
    for fid, label, _folders in FILTERS:
        count = sum(1 for i in images if fid in i["filters"])
        print("  %-11s %-16s %d" % (fid, label, count))
